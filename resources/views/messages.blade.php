        ...
        <div class="max-w-8xl  mx-auto lg:px-8 grid lg:gap-8 gap-4 lg:grid-cols-1 md:grid-cols-1 sm:grid-cols-1">
            <div
                ...
                <div class="flex flex-col sm:flex-row items-center justify-center py-4 relative">
                    ...
                    <form action="{{ Auth::user()->user_type === 'admin' ? route('admin.dashboard') : route('patient.dashboard') }}" method="GET">
                            @csrf
                            <button
                ...
                <div class="bg-white max-h-full   p-4 text-xl">
                    ...
                    <div class="overflow-y-auto p-2  h-screen flex flex-col md:flex-row gap-4">
                        <div class="bg-gray-100 border-r-4 border-teal-600 rounded-lg  lg:w-1/3 md:w-full ">
                            ...
                            <div class=" p-4">
                                ...
                                <div
                                    ...
                                    <x-message-modal :modalid="'createmodal'"
                                    :route="route('messages.create.post')"
                                    :userToEmail="$recipients ?? []"  {{-- Ensure it's always an array --}}
                                    ...
                                    @foreach ($messages as $message)
                                        ...
                                        <div class="message-item  border-1 rounded-lg shadow-lg p-4 {{ $bgclass }} border-l-4 border-teal-500 hover:bg-teal-100 transition-all duration-300 ease-in-out cursor-pointer"
                                            ...
                                            @php
                                                ...
                                                $recipientEmail = ($status === 'received') ? $message->sender->email : $message->receiver->email;
                                            @endphp
                                            <x-message-modal 
                                                :modalid="'replymodal'" 
                                                :route="route('messages.replies.post', $message->id)" 
                                                title="Reply Message"
                                                :userToEmail="$recipientEmail"
                                                :userFromEmail="Auth::user()->email"
                                                data-sender="{{ $message->sender->email }}" 
                                            />
                                        </div>
                            ...
                            document.addEventListener("DOMContentLoaded", function() {
                                document.querySelectorAll(".message-item").forEach(item => {
                                    item.addEventListener("click", function() {
                                        ...
                                        console.log(status);
                                        if (status === "received") { // Only update if status is 'received'
                                            fetch(`/update-seen-status/${messageId}?status=${status}`, {
                                                method: "POST",
                                                headers: {
