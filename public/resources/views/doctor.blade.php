            ...
            <div class="max-w-3xl mx-auto">
                <div class="bg-white rounded-2xl shadow-xl overflow-hidden hover-card">
                    <div class="flex flex-col md:flex-row">
                        <div class="md:w-1/2">
                            <img src="{{ $doctor->image && Storage::disk('public')->exists($doctor->image) ? asset('storage/' . $doctor->image) : asset('images/default-avatar.png') }}" alt="Dr. Ana Fatima Barroso"
                        ...
                        <div class="md:w-1/2 p-8">
                            ...
                            <div class="mt-8">
                                <a href="/appointment" class="inline-flex items-center px-6 py-3 bg-teal-500 text-white font-semibold rounded-lg 
                ...
                @foreach($personnel as $person)
                    <div class="group">
                        <div
                            ...
                            <img class="w-full h-64 object-cover object-center sm:h-48 md:h-56 lg:h-64"
                                src="{{ $person->image && Storage::disk('public')->exists($person->image) ? asset('storage/' . $person->image) : asset('images/logo.png') }}"
                                alt="{{ $person->name }} image">

                            <div class="p-4 pt-2 bg-teal-50 text-center">
                                <div
                                    class="w-12 h-12 bg-teal-100 rounded-full flex items-center justify-center mx-auto mt-4">
                                ...
                                <h3 class="text-lg font-semibold text-teal-800">{{ $person->name }}</h3>
                                <p class="text-sm text-teal-600 mb-4">{{ $person->position }}</p>
                                <p class="text-sm text-gray-600">
                                    {{ implode(' ', array_slice(explode(' ', $person->description), 0, 6)) }}{{ strlen($person->description) > 0 ? '...' : '' }}
                                </p>
                                    <!-- Added the SVG Icon or Avatar -->
                                <div class="mt-4 flex justify-center space-x-4">
                                    @if($person->facebook)
                                        <a href="{{ $person->facebook }}" class="text-gray-500 hover:text-teal-600"
                                            target="_blank">
                                    ...
                                    @endif
                                    @if($person->instagram)
                                        <a href="{{ $person->instagram }}" class="text-gray-500 hover:text-teal-600"
                                            target="_blank">
                                            <i class="fab fa-instagram"></i>
                                        </a>
                                    @endif
                                    @if($person->twitter)
                                        <a href="{{ $person->twitter }}" class="text-gray-500 hover:text-teal-600"
                                            target="_blank">
