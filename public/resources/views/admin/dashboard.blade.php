...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let appointmentsChart = new Chart(appointmentsCtx, {
        ...
        data: {
            labels: {!! json_encode($currentAppointmentData['labels'] ?? []) !!},
            datasets: [
                {
                    label: 'Pending',
                    ...
                    data: {!! json_encode($currentAppointmentData['pending'] ?? []) !!}
                },
                {
                    label: 'Approved',
                    ...
                    data: {!! json_encode($currentAppointmentData['approved'] ?? []) !!}
                },
                {
                    label: 'Attended',
                    ...
                    data: {!! json_encode($currentAppointmentData['attended'] ?? []) !!}
                }
            ]
        },
        options: {
    ...
    let patientRegistrationChart = new Chart(patientCtx, {
        ...
        data: {
            labels: {!! json_encode($currentPatientData['labels'] ?? []) !!},
            datasets: [
                {
                    label: 'New Patients',
                    ...
                    data: {!! json_encode($currentPatientData['registrations'] ?? []) !!}
                }
            ]
        },
        options: {
    ...
    document.getElementById('appointmentTimePeriod').addEventListener('change', function() {
        ...
        if (period === 'thisWeek') {
            updateAppointmentsChart(
                {!! json_encode($appointmentData['thisWeek']['labels'] ?? []) !!},
                {!! json_encode($appointmentData['thisWeek']['pending'] ?? []) !!},
                {!! json_encode($appointmentData['thisWeek']['approved'] ?? []) !!},
                {!! json_encode($appointmentData['thisWeek']['attended'] ?? []) !!}
            );
        } 
        else if (period === 'thisMonth') {
            updateAppointmentsChart(
                {!! json_encode($appointmentData['thisMonth']['labels'] ?? []) !!},
                {!! json_encode($appointmentData['thisMonth']['pending'] ?? []) !!},
                {!! json_encode($appointmentData['thisMonth']['approved'] ?? []) !!},
                {!! json_encode($appointmentData['thisMonth']['attended'] ?? []) !!}
            );
        }
        else if (period === 'last3Months') {
            updateAppointmentsChart(
                {!! json_encode($appointmentData['last3Months']['labels'] ?? []) !!},
                {!! json_encode($appointmentData['last3Months']['pending'] ?? []) !!},
                {!! json_encode($appointmentData['last3Months']['approved'] ?? []) !!},
                {!! json_encode($appointmentData['last3Months']['attended'] ?? []) !!}
            );
        }
        else if (period === 'last6Months') {
            updateAppointmentsChart(
                {!! json_encode($appointmentData['last6Months']['labels'] ?? []) !!},
                {!! json_encode($appointmentData['last6Months']['pending'] ?? []) !!},
                {!! json_encode($appointmentData['last6Months']['approved'] ?? []) !!},
                {!! json_encode($appointmentData['last6Months']['attended'] ?? []) !!}
            );
        }
        else if (period === 'thisYear') {
            updateAppointmentsChart(
                {!! json_encode($appointmentData['thisYear']['labels'] ?? []) !!},
                {!! json_encode($appointmentData['thisYear']['pending'] ?? []) !!},
                {!! json_encode($appointmentData['thisYear']['approved'] ?? []) !!},
                {!! json_encode($appointmentData['thisYear']['attended'] ?? []) !!}
            );
        }
    });
    ...
    document.getElementById('patientTimePeriod').addEventListener('change', function() {
        ...
        if (period === 'thisWeek') {
            updatePatientChart(
                {!! json_encode($patientData['thisWeek']['labels'] ?? []) !!},
                {!! json_encode($patientData['thisWeek']['registrations'] ?? []) !!}
            );
        }
        else if (period === 'thisMonth') {
            updatePatientChart(
                {!! json_encode($patientData['thisMonth']['labels'] ?? []) !!},
                {!! json_encode($patientData['thisMonth']['registrations'] ?? []) !!}
            );
        }
        else if (period === 'last3Months') {
            updatePatientChart(
                {!! json_encode($patientData['last3Months']['labels'] ?? []) !!},
                {!! json_encode($patientData['last3Months']['registrations'] ?? []) !!}
            );
        }
        else if (period === 'last6Months') {
            updatePatientChart(
                {!! json_encode($patientData['last6Months']['labels'] ?? []) !!},
                {!! json_encode($patientData['last6Months']['registrations'] ?? []) !!}
            );
        }
        else if (period === 'thisYear') {
            updatePatientChart(
                {!! json_encode($patientData['thisYear']['labels'] ?? []) !!},
                {!! json_encode($patientData['thisYear']['registrations'] ?? []) !!}
            );
        }
    });
