            ...
            <div class="flex flex-col gap-4 p-6">
                ...
                <div class="w-full">
                    ...
                    @if($modalid === 'replymodal')
                        <input type="email" name="user_to" value="{{ $userToEmail }}" readonly
                    ...
                    @else
                        <select name="user_to"
                            class="w-full border border-slate-400 rounded-md px-3 py-2 text-sm text-slate-700 bg-transparent focus:outline-none focus:border-slate-400 hover:border-slate-300 shadow-sm focus:shadow transition duration-300 ease-in-out">
                            @if(is_array($userToEmail) && count($userToEmail) > 0)
                                @foreach($userToEmail as $email)
                                    <option value="{{ $email }}">{{ $email }}</option>
                                @endforeach
                            @else
                                <option value="">No recipients found</option>
                            @endif
                        </select>
                    @endif
